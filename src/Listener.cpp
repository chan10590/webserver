...
Listener::Listener(EventLoop* evloop, uint16_t port, uint32_t ev)
    : m_evloop(evloop), m_port(port), m_event(ev) {
    ...
    m_listen_channel->setRDCallback(newconc_cb);
...
/*为新建的通信套接字设置回调*/
void Listener::setCommunCb(std::function<void(Socket*)> cb) { m_commun_cb = cb; }
}  // namespace web
